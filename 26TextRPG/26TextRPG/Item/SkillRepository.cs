...
namespace _26TextRPG
{
	internal class SkillRepository
	{
		...
		static SkillRepository()
		{
            ...
            {
				new Skill("배쉬",3,1,1.5,false),
				new Skill("슬래쉬", 5, 1, 2.0f, false),
				new Skill("방패 밀치기", 4, 2, 1.5f, false),
				new Skill("원드밀", 8, 1, 1.5f, true),
				new Skill("파이널 히트", 10, 3, 1.5f, false),
				//스킬 추가
			};
			Archer = new List<Skill>() // 궁수 스킬 리스트
            {
				new Skill("해드샷", 3, 1, 1.5f, false),
				new Skill("연속 사격", 5, 1, 2.0f, false), 
				new Skill("다중 화살", 5, 1, 1.3f, true), 
				new Skill("신속 발사", 3, 5, 2.0f, false), 
				new Skill("파이널 샷", 10, 1, 3.0f, false), 
                //스킬 추가
            };
			Wizard = new List<Skill>()  // 마법사 스킬 리스트
            {
				new Skill("매직 미사일", 4, 5, 0.5f, false), 
				new Skill("매직 에로우", 6, 5, 0.8f, false), 
				new Skill("매직 스피어", 3, 5, 1.1f, false), 
				new Skill("체인 라이트닝", 7, 5, 1.6f, true),
				new Skill("파이어 스톰", 10, 5, 2.0f, true), 
                //스킬 추가
            };
